...
/**
 * Copiloto IA de Territorios — Chat con acceso a la base de datos vía Grok (Puter.js).
 * El modelo ejecuta herramientas (tool calling) contra el endpoint seguro
 * /api/ai/query-territorios (solo lectura y aislado por congregación).
 */
...
function buildSystemPrompt(): string {
  ...
  const configuredPrompt = process.env.NEXT_PUBLIC_COPILOT_TERRITORIOS_SYSTEM_PROMPT;
  if (!configuredPrompt) {
    throw new Error("Falta configurar NEXT_PUBLIC_COPILOT_TERRITORIOS_SYSTEM_PROMPT en .env");
  }
...
async function executeToolCall(call: ToolCallRequest): Promise<string> {
  try {
    const res = await fetch("/api/ai/query-territorios", {
      method: "POST",
